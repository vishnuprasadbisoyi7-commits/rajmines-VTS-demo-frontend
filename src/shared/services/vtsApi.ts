...
export const vtsApi = {
  ...
  filterActiveTransmittingItems(items: LiveVehicleApiItem[]): LiveVehicleApiItem[] {
    ...
    if (maxTime === 0) return items;

    // Actively transmitting: updated within 2 hours of latest packet received across fleet
    const activeWindowMs = 2 * 60 * 60 * 1000;
    return items.filter((item) => {
      if (!item.updated_at) return false;
      const t = new Date(item.updated_at).getTime();
      return !isNaN(t) && (maxTime - t) <= activeWindowMs;
    });
  },
  ...
  async getVehicles(): Promise<Vehicle[]> {
    try {
      ...
      const liveApiRes = await this.getLiveVehicles(true);
      if (liveApiRes && Array.isArray(liveApiRes.data) && liveApiRes.data.length > 0) {
        const activeItems = this.filterActiveTransmittingItems(liveApiRes.data);
        if (activeItems.length > 0) {
          return activeItems.map(apiLiveVehicleToVehicle);
        }
      }

      // 2. Secondary fallback: recent telemetry records filtered by active timestamp
  ...
  // Used by Vehicle Replay to allow selecting from all registered vehicles
  async getAllVehicles(): Promise<Vehicle[]> {
    try {
      const liveApiRes = await this.getLiveVehicles(false);
      const liveList =
